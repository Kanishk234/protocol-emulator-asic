...
- [ ] `ARCHITECTURE.md` and `ISA.md` have **zero OPEN items**; the semantics section is reviewed by at least two people. *(Zero OPEN items: done 2026-09-23 by D-029 (`grep OPEN` finds only the intro's rule for new items). The R1 RTL spike found eight §14 gaps (G1–G8); all are answered in §14 (D-033). Still needed: the second person's review of §14.)*
...
- [ ] **R2 decided:** the latch-array test project passed precheck and `gl_test`, or the flop fallback is chosen and logged. *(In progress 2026-09-24: `spikes/r2_latch/`, branch `spike/r2-latch`, 3x2 with the whole R1 lane (D-032); local RTL + gate-level PASS, CI hardening pending.)*
