...
| 10 | LINE | [5] level, [4] override, [3] append CRC, [2] stuff, [1:0] readback | BITSYNC TX, in stream order: readback 0 off / 1 abort on mismatch / 2 report every bit; stuffing on/off; then optionally shift out the CRC. With [4]: bypass the queue and drive [5] for one bit, at the first bit start after the next sample point, unless this unit transmits the frame in progress |
<!-- /GENERATED:pin_commands -->
...
- **P21.** Frame start: at bus idle, a recessive-to-dominant edge on the sensed pin hard-syncs the bit clock (bit start := that clock) and starts a received frame. A `SYNC` in the TX stream makes the next TX bits a new frame: it starts at the first bit boundary after bus idle, hard-syncing to our own first bit, and emits EVENT `0x9001`. If another node's frame starts while ours is waiting and our first bit is dominant, ours joins that bit (wired-AND arbitration) and also emits `0x9001`.
- **P22.** Resync: on a recessive-to-dominant edge (or any edge with RESYNC = both) inside a frame, at most once per bit and not while this unit drives the dominant level in the current bit, the bit start moves toward the edge by at most SJW: later if the edge is before the sample point, earlier (shortening the bit) if after it.
...
- **P24.** Frame length and CRC: the CRC register (CRC_INIT at the frame start, MSB-first LFSR with CRC_POLY) takes every destuffed RX bit. Words of RX_NBITS (ORDER) are emitted as DATA. When the destuffed count reaches the frame's n (`FRAME`), the last partial word is emitted as EVENT if the CRC register equals CRC_RES, else as ERR `0x0www`; word framing and RX stuffing then stop, except that a stuff bit due right after bit n is still removed and checked. `FRAME` without [11] applies to the frame in progress (LATE if there is none or it is already past n); with [11], to the next frame.
- **P25.** TX queue: DATA tokens (NBITS or length-in-token, ORDER) and `LINE`/`SYNC` markers are queued; a TX token is taken only when no data bits are queued. At each bit start the next bit is driven (a stuff bit first if due); `LINE` markers take effect in stream order, and `LINE` [3] inserts the current CRC register (CRC_WIDTH bits, MSB first) at that point. With no bits queued the line is released (recessive). RX commands (`FRAME`, `SETN` rx, `LINE` [4]) are taken at once, even while TX data is queued.
- **P26.** Readback and override: for a bit we drive, readback mode 1 compares the sample with it; on a mismatch the unit emits EVENT `0x8000 | level << 14 | bit << 1`, clears its TX queue and discards DATA/`LINE` tokens until a `SYNC` or `WAIT`. Mode 2 emits `0xA000 | level << 14 | bit << 1` for every such bit. `LINE` [4] arms a one-bit override at the next non-stuff sample point; at the following bit start the unit drives level [5] for that bit, unless it is transmitting the frame in progress.
