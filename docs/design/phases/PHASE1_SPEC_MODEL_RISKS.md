...
1. Resolve every **OPEN** item in `ARCHITECTURE.md`:
   - token tag set;
...
6. Implement it from `ARCHITECTURE.md` and the YAML, token-level and cycle-accurate:
...
- [ ] `ARCHITECTURE.md` has **zero OPEN items**; the semantics section is reviewed by at least two people.
