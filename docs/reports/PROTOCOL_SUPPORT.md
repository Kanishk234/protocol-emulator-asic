...
| **CAN 2.0A** (stretch goal) | **Verified (model)** | `programs/can.trw`, 2 lanes (11 + 10 slots), 1 pin unit in BITSYNC mode (D-023). 125 k / 500 k / 1 Mbit/s; TX, RX, ACK, arbitration loss + host retry, CRC errors, ±0.4 % clock; reference node + sigrok `can`. Not yet: error frames and error counters (error-active/passive, bus-off), overload frames, extended 29-bit IDs, remote frames, CAN FD |
| **USB low-speed** (stretch, feasibility only) | Needs primitives: **NRZI codec**, **SE0 detection**, **complementary output pair** (BITSYNC already gives resync, 6-ones stuffing and CRC5/16) | 1.5 Mbit/s is 33 clocks per bit: comfortable. The work is the codec path and bus-turnaround timing. Largest effort of any row |
