...
| 3 | AND | A & B | d == 0 | Masking; bit test (replaces TSTZ) |
| 4 | OR | A \| B | d == 0 | Setting bits; placing constants (`zero \| K`) |
| 5 | XOR | A ^ B | d == 0 | Equality test (replaces CMPEQ); arbitration readback |
| 6 | SHL | A << B[3:0] | d == 0 | |
| 7 | SHR | A >> B[3:0] | d == 0 | |
...
| 10 | CMPM | A (pass-through) | (A & M) == V, where M, V = K[F[1:0]], K[F[3:2]] if F[4] else r[F[1:0]], r[F[3:2]] | Address match, multi-bit checks |
...
CONDITION (20 bits)
  [0]      V      slot valid
  [1]      U      urgent: pre-empts a waiting routine step (§5.3)
  [2]      SE     STATE-match enable
  [6:3]    SV     required STATE
...
ACTION (32 bits)
  ...
  [29:27]  ASRC   0-3 r0-r3 | 4 I0 head | 5 I1 head | 6 zero | 7 time
  [30]     DQ     dequeue the A input (ignored unless ASRC is I0/I1)
  [32:31]  BSEL   00 B = r[IMM[1:0]] | 01 B = IMM (zero-extended) | 10 B = K[IMM[1:0]] | 11 reserved
  [40:33]  IMM    8-bit immediate; also the field operand F
  [41]     NSE    next-STATE enable (static update)
  [45:42]  NS     next STATE
  [46]     DFE    write the flag result R
...
```
ALU     0 op[14:11] rd[10:9] ra[8:7] bs[6] b[5:0]
          B = bs ? zext(b) : r[b[1:0]]        F = bs ? zext(b) : r[b[1:0]][7:0]
          rd := d (write rd = ra for compare-only use); RZ := R
          op 14 (CALL) and 15 are reserved in routines (no nesting)

CTRL    1 sub[14:12] ...
  000   LDI   rd[11:10] imm[9:0]        rd := zext(imm10)
  001   LDIH  rd[11:10] imm[9:0]        rd := {imm[5:0], rd[9:0]}
  010   BR    cond[11:9] off[8:0]       cond: 000 always | 001 if RZ | 010 if !RZ | others reserved
  011   DJNZ  rd[11:10] off[9:0]        rd := rd - 1; branch if rd != 0 (RZ unchanged)
  100   LD    rd[11:10] ra[9:8] off[7:0] rd := SRAM[r[ra] + off]
  101   ST    rd[11:10] ra[9:8] off[7:0] SRAM[r[ra] + off] := rd
  110   OUT   port[11] tag[10:9] ra[8:7] enqueue r[ra] with tag on O0/O1; waits while the output is full
  111   SYS   kind[11:8] arg[7:0]
              0 NOP | 1 RET | 2 SETST v | 3 SETF f | 4 CLRF f | 5 TSTF f (RZ := f)
              6 CPYF f (f := RZ) | 7 GETT rd (rd := time) | 8 GETK rd, k (rd := K[k]) | 9-15 reserved
```
