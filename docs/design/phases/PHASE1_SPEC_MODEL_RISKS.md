...
- [ ] `ARCHITECTURE.md` and `ISA.md` have **zero OPEN items**; the semantics section is reviewed by at least two people. *(Zero OPEN items: done 2026-09-23 by D-029 (`grep OPEN` finds only the intro's rule for new items). Still needed: the second person's review of §14.)*
