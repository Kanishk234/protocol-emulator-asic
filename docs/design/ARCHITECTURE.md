...
| Pin units | 6 (U0–U5), each with a TX half and an RX half | U0–U1 full; U2–U5 lean, without PULSE, carrier or BITSYNC (D-040) |
...
U0 and U1 are **full** units. U2–U5 are **lean**: they have every core mode (LEVEL, SHIFT, CLKGEN, SHIFT_RX, LINKED_RX), the cursor, the event generator and pins A, B, C, S and N, but not PULSE, the carrier or BITSYNC (D-040). The generated table in §7.2 marks which units have each field. tripc rejects a configuration that uses a feature its unit lacks.
...
Each unit has a block of configuration registers in the host space (§9), written while the lanes are halted (§14 H1). The blocks are a latch array with no reset (D-038), so the host writes every unit's block before RUN, and they are write-only (D-039). On a lean unit the optional features' fields are not stored: writes to them are ignored, the hardware treats them as 0, and the missing modes act as LEVEL / off (D-040). The layout is generated from `spec/tripwire.yaml` (`pin_config`, D-036). Times are exact 16.8 fixed-point clock counts. The SHIFT_RX / BITSYNC sample point is stored as an offset in 1/256 clocks, not as a fraction of PERIOD, so its rounding is fixed by the host (P5). What each setting does is in §14 (P-rules) and in the DECISIONS entry that introduced it (D-010, D-011, D-013, D-014, D-016, D-019, D-020, D-023 to D-027). The model runs every configuration through this encoding (`tools/tripsim/pinregs.py`), so it only uses values the registers can hold.

<!-- GENERATED:pin_config -->
<!-- GENERATED by tools/gen/gen.py from spec/tripwire.yaml. DO NOT EDIT. Edit spec/tripwire.yaml. -->
Unit u's block: `0x3000 + u·0x20`; output owners: `0x30c0 + (pad − 8)`, `[2:0]` = unit, 7 = none.

Optional features (D-040); every other field and mode is on all units:
- **pulse** (units U0, U1): modes TXMODE pulse
- **carrier** (units U0, U1)
- **bitsync** (units U0, U1): modes TXMODE bitsync; RXMODE bitsync

Storage: latch array, no reset (D-038). Host-readable: no (D-039).

| Word | Bits | Field | Encoding | Units | Meaning |
|---|---|---|---|---|---|
| 0 | [2:0] | TXMODE | 0 level, 1 shift, 2 clkgen, 3 pulse, 4 bitsync | all | TX mode |
| 0 | [4:3] | RXMODE | 0 off, 1 shift_rx, 2 linked_rx, 3 bitsync | all | RX mode (bitsync with txmode bitsync) |
| 0 | [5] | ORDER | 0 lsb, 1 msb | all | bit order of shifted words |
| 0 | [6] | OD | 0/1 | all | open drain on pin A: 1 releases, 0 drives low |
| 0 | [7] | IDLE | value | all | idle level of pin A (BITSYNC: the recessive level) |
| 0 | [8] | AUTOREARM | 0/1 | all | SHIFT_RX re-arms after each word |
| 0 | [9] | RX_ECHO | 0/1 | all | keep RX words sampled while our own TX shifted (0: drop them) |
| 0 | [10] | TX_LENTOK | 0/1 | all | DATA carries its length: data[15:12] = n - 1 |
| 0 | [11] | TX_PRELOAD | 0/1 | all | linked TX: bit 0 at once when idle (SPI CPHA = 0) |
| 0 | [12] | STRETCH | 0/1 | all | CLKGEN waits for the line to read IDLE |
| 0 | [13] | NRZI | 0/1 | U0, U1 | BITSYNC NRZI line coding |
| 0 | [14] | OE_AUTO | 0/1 | U0, U1 | BITSYNC: drive the pads only while sending |
| 1 | [4:0] | PIN_A | pad number, 31 = none | all | drive and/or sample pad |
| 1 | [9:5] | PIN_B | pad number, 31 = none | all | link / condition pad |
| 1 | [10] | RX_EDGE | 0 rise, 1 fall | all | LINKED_RX samples on this edge of pin B |
| 1 | [12:11] | TX_EDGE | 0 none, 1 rise, 2 fall | all | linked TX shifts on this edge of pin B (none = timed) |
| 2 | [4:0] | PIN_C | pad number, 31 = none | all | select / frame pad |
| 2 | [5] | C_ACTIVE | value | all | pin C level that selects the unit |
| 2 | [6] | C_OE | 0/1 | all | drive pin A only while selected |
| 2 | [11:7] | PIN_S | pad number, 31 = none | all | sense pad, read instead of pin A |
| 3 | [4:0] | PIN_N | pad number, 31 = none | all | complement output of pin A |
| 3 | [5] | EV_PIN | 0 a, 1 c | all | event source pin |
| 3 | [7:6] | EV_EDGE | 0 none, 1 rise, 2 fall, 3 both | all | event on this edge |
| 3 | [9:8] | EV_QUAL | 0 none, 2 0, 3 1 | all | only while pin B is at this level (2 samples) |
| 3 | [10] | EV_RESET | 0/1 | all | an event restarts RX word framing |
| 4 | [15:0] + 5[7:0] | PERIOD | 16.8 clocks | all | bit period, 16.8 clocks, 1.0 to < 65536 |
| 5 | [15:8] | PRESC | value − 1 | all | clocks per tick, 1..256 |
| 6 | [15:0] + 7[7:0] | SAMPLEOFS | 16.8 clocks from the bit start | all | sample point after the bit start / start edge, 16.8 clocks |
| 7 | [11:8] | NBITS | value − 1 | all | TX shift length, 1..16 |
| 8 | [4:0] | RX_NBITS | value | all | RX word length, 1..16 (0 = NBITS) |
| 8 | [9:5] | RX_NBITS2 | value | all | two-phase framing second length, 0 = off |
| 9 | [11:0] | SYM0_T1 | value | U0, U1 | bit 0: first level, ticks |
| 9 | [12] | SYM0_FIRST | value | U0, U1 | bit 0: first level |
| 10 | [11:0] | SYM0_T2 | value | U0, U1 | bit 0: second level, ticks |
| 11 | [11:0] | SYM1_T1 | value | U0, U1 | bit 1: first level, ticks |
| 11 | [12] | SYM1_FIRST | value | U0, U1 | bit 1: first level |
| 12 | [11:0] | SYM1_T2 | value | U0, U1 | bit 1: second level, ticks |
| 13 | [15:0] + 14[7:0] | CARRIER | 16.8 clocks | U0, U1 | carrier period, 16.8 clocks (0 = off) |
| 15 | [15:0] + 16[7:0] | SJW | 16.8 clocks | U0, U1 | max resync step, 16.8 clocks (0 = hard sync only) |
| 16 | [12:8] | IDLE_BITS | value | U0, U1 | recessive samples that make the bus idle, 1..31 |
| 16 | [13] | RESYNC | 0 dom, 1 both | U0, U1 | resync on recessive-to-dominant edges only, or both |
| 16 | [15:14] | DELIM | 0 none, 1 flag, 2 se0 | U0, U1 | frame delimiter |
| 17 | [3:0] | STUFF_N | value | U0, U1 | stuff after n equal bits, 0 = off |
| 17 | [5:4] | STUFF_LVL | 0 none, 2 0, 3 1 | U0, U1 | stuff only runs of this level |
| 17 | [10:6] | CRC_WIDTH | value | U0, U1 | CRC width, 0 = none, 1..16 |
| 17 | [15:11] | CRC_SKIP | value | U0, U1 | RX CRC starts after this many frame bits |
| 18 | [15:0] | CRC_POLY | value | U0, U1 | CRC polynomial (MSB-first LFSR) |
| 19 | [15:0] | CRC_INIT | value | U0, U1 | CRC register at frame start |
| 20 | [15:0] | CRC_RES | value | U0, U1 | expected RX CRC register after the frame |
| 21 | [15:0] | CRC_XOR | value | U0, U1 | XOR-ed into the TX CRC when appended |
<!-- /GENERATED:pin_config -->
...
| 0x1000–0x1FFF | Reflex slots: `lane[9:8] slot[7:4] word[1:0]` (4 × 16-bit words per slot); slot index 12, words 0–3 = K0–K3 (§14 H1). Write-only (D-039) |
| 0x2000–0x20FF | Fabric consumer port registers (sel, en, mode) |
| 0x3000–0x30FF | Pin unit configuration, `0x3000 + u·0x20` (22 words used, §7.2), and output owners at `0x30C0 + (pad − 8)`. Unit blocks write-only (D-039); owners readable |
...
**Debug:**
- Any lane can be halted, single-stepped (exactly one EVAL/EXEC) and inspected: registers, state, flags, routine state and channels. Slots, K and the pin configuration are write-only (D-039).
...
| Pin units ×6 | ~0.55K flops (phase 1 guess, far too low: see the note below) |
| Helpers + host + time base | ~0.5K flops |
| SRAM 512x16 macro | 45.3K µm² |
| **Total** | **~480–520K µm², ~53–57% of the 6x4 core** |

**Superseded by the pre-RTL estimate** (`docs/reports/AREA_ESTIMATE.md`, 2026-09-25): the spec as frozen came to ~1.03M µm² placed (115 % of the core), mostly pin units at ~84K µm² each. With D-038–D-040 it is ~781K (87 %): a full unit ~72K, a lean one ~41K before layout. The budget is still open (phase 2 task 2.0).
...
### Reset and loading
- **H1.** After reset every lane is halted (RUN = 0). Flops (registers r0–r3, STATE, flags, PEND, RB, RIR state, producers, ports) reset to 0. Slot and K latches have no reset, so slot contents (including `V`) and K0–K3 are undefined. Before setting RUN the host writes all 12 slots and K0–K3 of a lane (52 words: unused slots with `V = 0`), and any nonzero initial r0–r3 or STATE through the lane register block (§9) (G7, D-035 E1/E2). The pin configuration latches have no reset either (D-038): the host also writes the block of every pin unit, units not in use with the default (modes off, pads 31 = none). Output owners are flops that reset to none, so an unwritten unit never drives a pad. `tripc.load` does all of this.
...
- **P8.** Event generator (D-013): an EVENT `{new level of A, time[14:0]}` is emitted when the synchronised pin A shows EV_EDGE, and, if EV_QUAL is set, pin B was at EV_QUAL in both this clock and the previous one. It uses this clock's RX load; a sample in the same clock still enters framing, and only a word it completes loses the load (P38, D-041). With EV_RESET it restarts word framing (phase 0, no bits). The time counts PRESC ticks (D-024), so with PRESC > 1 long pulses fit in 15 bits: it is a per-unit 15-bit tick counter, +1 every PRESC clocks from reset (the model computes `(cycle div PRESC) mod 2^15`). Pin units are configured while their lanes are halted, before RUN; the RTL restarts the prescaler and counter on a PRESC write, which matches the model when PRESC is written before any event matters (D-035 J). At most one RX load happens per clock (P19).
...
Pin rules added with the pin-unit RTL (the gaps P-G1–P-G23 found by writing `trw_pin_unit` from these documents, `docs/reports/PIN_UNIT_RTL.md` §6; answered in D-041):
- **P31.** Outputs (P-G1, P-G2). After reset and after a configuration, the TX output register holds IDLE with OE = 1; the output owners (reset to none) keep a unit off the pads until the host assigns them. Pin N drives the complement of pin A's value *before* open drain (the carrier included), with pin A's output enable before open drain (OE and C_OE). OD applies to pin A only. During SE0 both are low (P29).
- **P32.** Cursor range (P-G3). The cursor stays within a bounded distance of the present, counted in ticks of PRESC clocks. When a LEVEL, OE, GAP, SAMPLE or SETN rx is taken in clock n, a cursor more than 32 767 ticks before n first moves up to n − 32 767 ticks (its fraction cleared). A LEVEL, OE, GAP or SAMPLE that would put the cursor more than 32 768 ticks after n is not taken until it fits. Timed shifts, CLK bursts and PULSE words are not limited: their own timers carry the time, and they end at the present (P3).
- **P33.** CLKGEN timing (P-G4). The k-th level change of a burst is at `start + k·PERIOD/2`, computed exactly (to 1/512 clock), and happens at the edge of the clock that contains it. So an odd PERIOD in 1/256 clocks does not drift. With STRETCH, each IDLE half is timed from the clock in which the line reads IDLE (P11). After the burst, `cursor` is the time of the final release (without STRETCH, `start + n·PERIOD`).
- **P34.** Tokens a mode does not use (P-G5, P-G6, P-G7). `CLK` with n = 0, `CLK` outside CLKGEN, DATA and EVENT tokens at a CLKGEN unit, and EVENT tokens at a SHIFT or PULSE unit are taken and ignored. CLKGEN needs PERIOD ≥ 2 clocks (tripc and the model reject less). A `CLK` taken in any clock of a burst, up to and including the clock of its final release, extends the burst: the next IDLE half starts at that release, as if n had been larger. The burst count is 9 bits, so a `CLK` that would take it above 511 periods waits.
- **P35.** WAIT (P-G8, P-G21). In the clock the WAIT edge appears, `cursor := n + 1`, and the next token may be taken in that same clock. `WAIT` [1] (sample point) outside BITSYNC is ignored: the unit waits for the [0] edge.
- **P36.** Linked TX and selection (P-G9). A linked shift moves only on TX_EDGEs in clocks in which the unit is selected (P15, pin C as synchronised in that clock), so a preloaded word waits through other targets' clocks. When a DATA token is taken with TX_PRELOAD in clock n, bit 0 goes out at edge n + 1, and a TX_EDGE seen in clock n or n + 1 is used by the preload (otherwise bit 1 would land on the same edge as bit 0 and hide it). Bit 1 waits for the next TX_EDGE.
- **P37.** SHIFT_RX restart (P-G10). Without AUTOREARM, SHIFT_RX stops after its word. A `SETN` rx (P18) returns SHIFT_RX to "wait for IDLE, then a start edge", whether it was stopped or in the middle of a word.
- **P38.** One framing bit per clock (P-G11, P-G12; this replaces "a sample due in the same clock is skipped" in P8). At most one bit enters RX framing per clock. The LINKED_RX / SHIFT_RX sample has priority; a SAMPLE bit due in the same clock is dropped and sets OVERRUN. A sample taken in the clock of an EVENT still enters framing (after the event's EV_RESET, if any); only a word it completes loses that clock's load (discarded, OVERRUN, P19).
- **P39.** Timed RX actions (P-G13). `SETN` rx and `SAMPLE` are pending actions for P3, like LEVEL: later tokens wait for them. A SAMPLE whose time is already past happens at the earliest edge and does not set LATE.
- **P40.** Taint (P-G14; clarifies P13). A sample is tainted while the unit's TX output register holds one of its own *shifted* bits (SHIFT or PULSE), whatever OD and OE are. The return to IDLE, a LEVEL change and an abort end it. CLKGEN levels never taint.
- **P41.** RX word lengths (P-G17). RX_NBITS = 0 means the configured NBITS, not a `SETN` tx override. RX_NBITS and RX_NBITS2 values 17–31 mean 16, as for `SETN` (P4); tripc and the model reject them.
- **P42.** Unattached pins and pad codes (P-G18). Pad codes 24–30 act as 31 (not attached); tripc and the model reject them. An unattached pin A (and S) reads IDLE, pin B reads 0, an unattached pin C means always selected (P15), and an unattached pin N is not driven.
- **P43.** Unassigned enum codes (P-G20). A configuration enum code with no name acts as the field's code 0: EV_QUAL 1 as none, TX_EDGE 3 as none, DELIM 3 as none, STUFF_LVL 1 as none, TXMODE 5–7 as LEVEL. The tools never write them.
- **P44.** Linked mode and other tokens (P-G22). A token other than DATA is taken only once all linked bits are out (P6). On one edge a linked bit beats a LEVEL, and a LEVEL beats a pending return to IDLE.
- **P45.** LEVEL-mode DATA and EVENT (P-G23; completes P12) set `cursor :=` their action time, like LEVEL with delay 0.
