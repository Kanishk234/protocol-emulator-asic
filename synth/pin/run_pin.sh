...
#
# Usage: [FRAC=4] synth/pin/run_pin.sh [period_ns]    (default 20; needs the liberty and OpenSTA cached by
#                                             spikes/r1_lane/run_r1.sh)
# Output: synth/pin/build/ (FRAC 8) or build/frac<N>/ (git-ignored): netlists, stat_*.txt, sta_*.txt, summary.txt
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(cd "$HERE/../.." && pwd)"
cd "$HERE"
PERIOD="${1:-20}"
FRAC="${FRAC:-8}"                        # time fraction bits (8 = the spec; 4 = measurement variant)
B=build; [ "$FRAC" = 8 ] || B="$B/frac$FRAC"
...
# synth <label> <top> <chparam-args> <files>: flat area; writes $B/stat_<label>.txt
synth() {
  local label=$1 top=$2 par=$3; shift 3
  yosys -q -l "$B/yosys_$label.log" -p "read_liberty -lib $TYP; read_verilog -DSYNTHESIS -I$ROOT/src $*; \
    hierarchy -top $top $par; synth -top $top -flatten; dfflibmap -liberty $TYP; abc -liberty $TYP; opt_clean; \
    tee -o $B/stat_$label.txt stat -liberty $TYP; write_verilog -noattr -noexpr $B/net_$label.v" \
    >/dev/null 2>&1 || { tail -20 "$B/yosys_$label.log"; exit 1; }
  sed -i 's/^\( *\)wire signed /\1wire /' "$B/net_$label.v"
}
area()  { awk '/Chip area for module/ {a=$NF} END {print a}' "$B/stat_$1.txt"; }
cells() { awk '$NF=="cells" || $2=="cells" {c=$1} END {print c}' "$B/stat_$1.txt"; }
count() { awk -v p="$2" '$0 ~ p {s+=$1} END {print s+0}' "$B/stat_$1.txt"; }

SUMMARY=$B/summary.txt
printf "%-12s %8s %10s %6s %7s %5s\n" block cells area_um2 flops latches icg > "$SUMMARY"
for f in 0 1; do
  synth "cfg$f"  trw_pin_cfg  "-chparam FULL $f" "$CFG"
  synth "unit$f" trw_pin_unit "-chparam FULL $f -chparam FRAC $FRAC" "$UNIT"
  synth "meas$f" trw_pin_meas "-chparam FULL $f -chparam FRAC $FRAC" "$ALL"
done
...
for f in 0 1; do
  yosys -q -p "read_liberty -lib $TYP; read_verilog -DSYNTHESIS -I$ROOT/src $UNIT; hierarchy -top trw_pin_unit -chparam FULL $f -chparam FRAC $FRAC; \
    synth -top trw_pin_unit; dfflibmap -liberty $TYP; abc -liberty $TYP; opt_clean; \
    tee -o $B/stat_hier_unit$f.txt stat -liberty $TYP" >/dev/null 2>&1
done
